...
struct ConsolManager {
    ...
    menuLoop:
        while true {
            ...
            case BankStatus.opening.menu :
                let bank = Bank()
                let totalCustomer = bank.executeWork(workTime: time)
                let totalTime = time * Double(totalCustomer)
                printCloseMessage(totalCustomer: totalCustomer, time: totalTime)
    ...
    private func printCloseMessage(totalCustomer: Int, time: Double) {
